...
// POST BLOG
blogRouter.post("/blog", auth, upload.single("file"), async (req, res) => {
  try {
    ...
    const model = { title, description, teaser };
...
// PUT BLOG
blogRouter.put("/blog", auth, upload.single("file"), async (req, res) => {
  try {
    const { id, title, description, teaser } = req.body;
    ...
    const model = { id, title, description, teaser };
